...
using namespace std;
	...
	istream& operator>> (istream& in,electronics& a )	//Change made at 10:35 p.m
	{
		...
		gotoxy(posx, posy++);
		cout << "Enter Title of Product : "; fflush(stdin); getstring(c);
		a.Title =c ;
		gotoxy(posx, posy++);
		cout<<"Enter Rate of Product : ";b=getf(0, 300000);
		a.Rate=b;
		gotoxy(posx, posy++);
		cout<<"Enter Cost of Product : ";b=getf(0, 300000);
		a.Cost=b;
		gotoxy(posx, posy++); 
		...
		a.Barcode=c ;
		gotoxy(posx, posy++);
		cout << "Enter Warranty of Product : "; fflush(stdin); getstring(c);
		a.Warranty=c ;
		gotoxy(posx, posy++);
		cout << "Enter Manufacturer of Product : "; fflush(stdin); getstring(c);
		a.Manufacturer_Supplier=c ;
		gotoxy(posx, posy++);
		cout << "Enter Description of Product : "; fflush(stdin); getstring(c);
		a.Description =c ;
		gotoxy(posx, posy++);
		return in;
...
// Constructors
	...
	electronics :: electronics (float rate,float cost,float quantity,string barcode,string warranty,string manu_supp,string title ,string description) 
	{
		ID=inv_count++;
		Rate = rate ;
		Cost = cost;
...
// Copy Constructor
	...
	void electronics :: setCost(float a) 
	{
		Cost =a;
	}
	...
	void electronics :: setWarranty (string a) 
	{
		Warranty = a;
	}
	...
	void electronics::setAll()
	{
		...
		Barcode = c;
		gotoxy(posx, posy++);
		cout << "Enter Warranty of Product : "; fflush(stdin); cin >> c;
		Warranty = c;
		gotoxy(posx, posy++);
		cout << "Enter Manufacturer of Product : "; fflush(stdin); getstring(c);
	...
	string electronics :: getWarranty () 
	{
		return  Warranty;
	}
	...
	case 1:
		...
		myfile<<"ID,Rate, Cost , Quantity, Bar Code , Manufacturer , TItle,Discription"<<endl;
		/*E(0,0)="ID";
		E(0,1)="Rate";
		E(0,2)="Quantity";
		E(0,3)="Bar Code";
		...
		myfile<<tempelec.getID()<<","<<tempelec.getRate()<<","<<tempelec.getCost()<<","","<<tempelec.getQuantity()<<","<<tempelec.getBarcode()<<","<<tempelec.getWarranty()<<","<<tempelec.getManufacturer_Supplier()<<","<<tempelec.getTitle()<<","<<tempelec.getDescrpition()<<endl;
