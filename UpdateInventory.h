...
string s2 = pp.str();
	...
	//sQuantity = sprintf(Cquantity, "%.4g", Quantity);
		...
		Edit_Electronics.setoptions(1, Title);
		Edit_Electronics.setoptions(2, s1);
		Edit_Electronics.setoptions(3, s2);
		Edit_Electronics.setoptions(4, Barcode);
		Edit_Electronics.setoptions(5, Manufacturer);
		Edit_Electronics.setoptions(6, Description);
		Edit_Electronics.setoptions(7, "\n\n\n\t\t\t\t\t\t\t\t\t\t\t\tDONE");

		switch (Edit_Electronics.updatemenu(x, y+1))
		{
		case 1:
			gotoxy(x, y+1); cout << "               ";
			gotoxy(x, y+1); fflush(stdin); getstring( Title); break;
		case 2:
			gotoxy(x, y+2); cout << "               ";
		...
		case 4:
			gotoxy(x, y+4); cout << "               ";
			gotoxy(x, y+4); fflush(stdin); getstring( Barcode);  break;
		case 5:
			gotoxy(x, y+5); cout << "               ";
			gotoxy(x, y+5); fflush(stdin); getstring( Manufacturer);  break;
		case 6:
			gotoxy(x, y+6); cout << "               ";
			gotoxy(x, y+6); fflush(stdin); getstring( Description);  break;
		case 7:
			Done = true;
		}
...
bool update_electronics()
{
	...
	while (1)
	{
		...
		case 2:	Edit_electronics_Item(file, E);	 break;
...
void Edit_sports_Item(fstream& file, sports& E1)
{
	int x=100, y=30, z=20;
	...
	//file.open("sports.dat", ios::in|ios::in|ios::binary);
	int  limit = 0 + E.getCount();
	gotoxy(x, y); 
	//file.open("sports.dat", ios::binary|ios::end|ios::beg);
	file.seekg((ID-1000)*sizeof(sports), ios::beg);
	if(file.fail())
	{ cout<<"Could not find location";file.clear();return ;
	}
	E.read(file);
...
string s2 = pp.str();
	...
	bool Done=false;
	Menu Edit_Sports(7);
	Edit_Sports.changecolour(121,121);
	while (!Done)
	{
		ss.str("");pp.str("");
		ss.clear();pp.clear();
	...
	//sQuantity = sprintf(Cquantity, "%.4g", Quantity);
		...
		Edit_Sports.setoptions(1, Title.c_str());
		Edit_Sports.setoptions(2, s1);
		Edit_Sports.setoptions(3, s2);
		Edit_Sports.setoptions(4, Barcode.c_str());
		Edit_Sports.setoptions(5, Manufacturer.c_str());
		Edit_Sports.setoptions(6, Description.c_str());
		Edit_Sports.setoptions(7, "\n\n\n\t\t\t\t\t\t\t\t\t\t\t\tDONE");

		

		switch (Edit_Sports.updatemenu(x, y+1))
		{
		case 1:
			gotoxy(x, y+1); cout << "               ";
			gotoxy(x, y+1); fflush(stdin); getstring( Title);  break;
	...
	E.setTitle(Title); E.setRate(Rate); E.setQuantity(Quantity);
			E.setBarcode(Barcode); E.setManufacturer_Supplier(Manufacturer);
			E.setDescription(Description);
	file.seekp((ID - 1000)*sizeof(sports), ios::beg);
	if(file.fail())
	{ cout<<"Could not find location";file.clear();return ;}
		E.write(file);
}
...
bool update_sports()
{
	...
	do
	{
		...
		case 2:	gotoxy(wherex(), wherey()+5);Edit_sports_Item(file, E); break;
...
void Edit_stationery_Item(fstream& file, stationery& E1)
{
int x=100, y=30, z=20;
	...
	gotoxy(x, y); 
	//file.open("sports.dat", ios::binary|ios::end|ios::beg);
	file.seekg((ID-2000)*sizeof(stationery), ios::beg);
	if(file.fail())
	{ cout<<"Could not find location";file.clear();return ;
	}
	E.read(file);
...
string s2 = pp.str();
	...
	bool Done=false;
	Menu Edit_Stationery(7);
	Edit_Stationery.changecolour(121,121);
	while (!Done)
	{
		ss.str("");pp.str("");
		ss.clear();pp.clear();
	...
	//sQuantity = sprintf(Cquantity, "%.4g", Quantity);
		...
		Edit_Stationery.setoptions(1, Title.c_str());
		Edit_Stationery.setoptions(2, s1);
		Edit_Stationery.setoptions(3, s2);
		Edit_Stationery.setoptions(4, Barcode.c_str());
		Edit_Stationery.setoptions(5, Manufacturer.c_str());
		Edit_Stationery.setoptions(6, Description.c_str());
		Edit_Stationery.setoptions(7, "\n\n\n\t\t\t\t\t\t\t\t\t\t\t\tDONE");

		

		switch (Edit_Stationery.updatemenu(x, y+1))
		{
		case 1:
			gotoxy(x, y+1); cout << "               ";
			gotoxy(x, y+1); fflush(stdin); getstring( Title);  break;
	...
	E.setTitle(Title); E.setRate(Rate); E.setQuantity(Quantity);
			E.setBarcode(Barcode); E.setManufacturer_Supplier(Manufacturer);
			E.setDescription(Description);
	file.seekp((ID - 2000)*sizeof(stationery), ios::beg);
	if(file.fail())
	{ cout<<"Could not find location";file.clear();return ;}
		E.write(file);}
...
//	Update_Stationery.setoptions(5, "Display An Item");				// !!! Temp !!!
	...
	do
	{
		...
		case 2:	Edit_stationery_Item(file, E);	 break;
